...
export default function makeProgram(program) {
  program
    ...
    .option('--diff', 'Output diff of changes after optimization to stderr')
    .option(
      '--diff-context <INTEGER>',
      'Set number of context lines for diff output (default: 3)',
    )
    .action(action);
}
...
async function action(args, opts, command) {
  ...
  if (opts.diffContext != null) {
    const number = Number.parseInt(opts.diffContext, 10);
    if (Number.isNaN(number) || number < 0) {
      console.error(
        "error: option '--diff-context' argument must be a non-negative integer number",
      );
      process.exit(1);
    } else {
      opts.diffContext = number;
    }
  }
  ...
  if (opts.diff) {
    config.diff = true;
    config.diffContext = opts.diffContext != null ? opts.diffContext : 3;
  }
...
function processSVGData(config, info, data, output, input) {
  ...
  if (config.diff) {
    ...
    if (diff) {
      if (diffOutputCount > 0) {
        process.stderr.write('\n');
      }
      process.stderr.write(`--- ${filename}\n+++ ${filename}\n`);
      const diffLines = diff.split('\n').slice(2).join('\n');
      process.stderr.write(diffLines + '\n');
      diffOutputCount++;
    }
  }
